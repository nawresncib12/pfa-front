      ...
      <div>
        ...
        <S.PillsContainer>
          {recipe.cautions.map((caution) => (
            <Pill key={"caution_" + caution} label={caution} color={"yellow"} />
          ))}
          {recipe.cuisineType.map((cuisine) => (
            <Pill
              key={"cuisine_" + cuisine}
              label={capitalizeFirstLetter(cuisine)}
              color={"purple"}
            />
          ))}
          {recipe.dishType.map((dish) => (
            <Pill key={"dish_" + dish} label={capitalizeFirstLetter(dish)} color={"purple"} />
          ))}
          {recipe.dietLabels.map((diet) => (
            <Pill key={"diet_" + diet} label={capitalizeFirstLetter(diet)} color={"purple"} />
          ))}
        </S.PillsContainer>
        <S.RecipeInfo>
          <S.RecipeInfoItem>
        ...
        <S.PillsContainer>
          {recipe.healthLabels.map((health) => (
            <Pill key={"health_" + health} label={capitalizeFirstLetter(health)} color={"green"} />
          ))}
        </S.PillsContainer>
      </div>
    </S.RecipePageContainer>
